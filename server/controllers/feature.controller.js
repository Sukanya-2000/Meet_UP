...
export const getSharedDatePlan = async(req,res)=>{const plan=await DatePlan.findOne({shareToken:req.params.token,status:{$nin:['draft']}}).select('-trustedContacts.phone -trustedContacts.email').lean();if(!plan){res.status(404);throw new Error('Shared date plan not found');}res.json({success:true,plan});};
export const transitionDatePlan=async(req,res)=>{const allowed=['shared','checked-in','arrived','emergency','completed','cancelled'];if(!allowed.includes(req.body.status)){res.status(400);throw new Error('Invalid date status');}const at=new Date();const timestamps={arrived: 'arrivedAt',emergency:'emergencyAt',completed:'finishedAt',cancelled:'cancelledAt'};const update={$set:{status:req.body.status,liveNote:String(req.body.note||'').slice(0,500)},$push:{history:{status:req.body.status,note:String(req.body.note||'').slice(0,500),at}}};if(timestamps[req.body.status])update.$set[timestamps[req.body.status]]=at;const plan=await DatePlan.findOneAndUpdate({_id:req.params.id,userId:req.user._id},update,{new:true,runValidators:true});if(!plan){res.status(404);throw new Error('Date plan not found');}req.app.get('io')?.to(`user:${req.user._id}`).emit('date-plan:updated',plan);res.json({success:true,plan});};
